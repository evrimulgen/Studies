...
int sommeNoeuds(ArbreBin A){
  ...
  /* A COMPLETER */
  if (A == NULL) return 0; 
  return sommeNoeuds(A->sad)+sommeNoeuds(A->sag)+A->info;
  }
...
int profMinFeuille(ArbreBin A){
  ...
  assert(A!=NULL);
  /* A COMPLETER */
  if (A->sag != NULL && A->sad != NULL)return min(profMinFeuille(A->sag),profMinFeuille(A->sad))+1;
  else if(A->sag != NULL) return profMinFeuille(A->sag)+1;
  else if(A->sad != NULL) return profMinFeuille(A->sad)+1;
...
int main(int argc, char *argv[]){
  ...
  case 1 :
    ...
    cout << "Somme des noeuds de l'arbre :"<< sommeNoeuds(B) << endl;
    cout << "Profondeur minimum des feuilles de l'arbre : " << profMinFeuille(B) << endl;
