...
void affichageArbre(int n,coord point[],int  arbre[][2],string name)
{
                                                                // les points et l'arbre de Kruskal.
 ofstream output;
 output.open(name.c_str(),ios::out);
...
main()
{
  ...
  int pere[N]={-1};                     // La relation de filiation de l'arbre de Dijkstra.
  ...
  affichageArbre(n,point,arbre,"arbre.ps");
