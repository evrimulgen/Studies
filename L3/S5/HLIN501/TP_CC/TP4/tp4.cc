...
void pointRandom2(int n,coord point[])
{
...
void voisins60(int n,coord point[],vector<int> voisin[],int &m)
{
    int nb_aretes=0;
    for(int i=0;i<n;i++)
        {
            for(int j=0;j<n;j++)
              {
                ...
                if( i != j)
                {
                    if(!isin(voisin,i,j) ||  (point[j].abs == point[i].abs+1 && point[i].ord==point[j].ord) || (point[j].ord == point[i].ord+1 &&  point[i].abs==point[j].abs) )
                    {
                        cout << mydistance(point[i],point[j]) << endl;
                        voisin[i].push_back(j);
                        voisin[j].push_back(i);
...
main()
{
  ...
  pointRandom2(n,point);
  for(int i=0;i<n;i++)
    cout <<"(" << point[i].abs <<","<< point[i].ord<<")"<<endl;

  voisins60(n,point,voisin,m);
